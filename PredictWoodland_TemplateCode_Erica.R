...
### BGC v10 linework
BGCv10 <- readShapePoly("BEC_SpatialData\\BEC_POLY_polygon_analysis.shp")
projection(BGCv10) <- P4S.AEA # tell R the projection of the shapefile (this is known via the shapefile metadata)


### create a study area for the analysis
...
pts <- spTransform(pts, P4S.AEA) #project to albers
# BGCv10.pts <- over(pts,BGCv10)  ##extract the BGCv10 attributes to the points (this takes a while, like 30ish minutes)
# write.csv(BGCv10.pts,"InputData\\BGCv10.pts.csv", row.names=FALSE) #write to csv so that you don't have to do the overlay more than once. 
BGCv10.pts <- read.csv("InputData\\BGCv10.pts.csv") #if this file has not already been created, then make it with the two rows above. 
pts <- as.data.frame(pts)[,3:5]
pts <- data.frame(id1=BGCv10.pts$MAP_LABEL, id2=BGCv10.pts$PHASE, lat=pts$y, lon=pts$x, el=pts[,3]) ## create the climateNA input file
write.csv(pts,"InputData\\pts.csv", row.names=FALSE)
...
grid.ref <- read.csv("InputData\\pts_Normal_1971_2000Y.csv", strip.white = TRUE, na.strings = c("NA","",-9999) )
